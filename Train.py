...
captcha_url = "https://its.bochk.com/servlet/IbsVerifyImageDisplayServlet"
captcha_length = 4
    ...
    def run(self):
        while True:
            ...
            while solver is None or len(solver.char_areas) != captcha_length:
                ...
                print(len(solver.char_areas))
            buffer.seek(0)
            captcha_result = solver.get_result()
            CaptchaGatherThread.captchas.put({"captcha": Image.open(
                buffer), "solver": solver, "guess": captcha_result})
            print('Put')
...
class CaptchaInputWindow(object):
    def __init__(self, parent):
        ...
        self.yes_button = tkinter.Button(
            self.parent, text="Fucking yes!", width=10, command=self.yes)
        self.yes_button.pack()
        self.entry_send = tkinter.Button(
            self.parent, text="Train", width=10, command=self.entry_callback)
        self.entry_send.pack()
        self.skip_button = tkinter.Button(
            self.parent, text="Skip", width=10, command=self.skip)
    ...
    def another(self):
        ...
        # self.interactive.set("Thought: {0}, Trained: {1}".format(
        # self.captcha_guess, solver_queue["answer"]))
        # self.another()
    ...
    def entry_callback(self):
        ...
        self.interactive.set("Thought: {0}, Trained: {1}".format(
            self.captcha_guess, self.entry.get()))
        self.another()
    ...
    def yes(self):
        ...
        self.interactive.set(
            "Answer: {0} - I am so awesome.".format(self.captcha_guess))
        self.another()
